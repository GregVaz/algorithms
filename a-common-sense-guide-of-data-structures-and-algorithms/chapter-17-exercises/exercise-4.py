...
class Trie:
    ...
    def autocorrect(self, word):
        ...
        if currentNode is None:
            return self.autocorrect(word[:-1])
        for key, _ in currentNode.children.items():
            if key == "*":
                return word
            else:
                possibleWords = self.collectAllWords(currentNode, word)
                if len(possibleWords) > 0:
                    return possibleWords[0]
                else:
                    self.autocorrect(word[:-1])
...
print(autocomplete.autocorrect('tankyou'))
print(autocomplete.autocorrect('todeyasd'))
